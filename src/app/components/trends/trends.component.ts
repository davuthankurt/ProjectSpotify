...
export class TrendsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getMultipleRandom(this.spotifyIds).forEach((id) => {
      forkJoin([this.spotifyService.getArtist(id).pipe()]).subscribe(
        (result: any) => {
  ...
  getMultipleRandom(arr) {
    const shuffled = [...arr].sort(() => 0.5 - Math.random()).slice(0, 8);
